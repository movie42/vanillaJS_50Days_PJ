...
var handleNumber = function handleNumber() {
  ...
  while (numberAry.length < 7) {
    var _number = Math.floor(Math.random() * (46 - 1)) + 1;

    if (i < 1) {
      numberAry.push(_number);
      i++;
    } else {
      var isSame = function isSame(number) {
        for (var j = 0; j < numberAry; j++) {
...
var paintNumber = function paintNumber() {
  var numberNodes = numbersContainer.childNodes;
  var numbers = handleNumber();
  numberNodes.forEach(function (value) {
  ...
  for (var _i = 0; _i < numbers.length; _i++) {
    for (var j = 0; j < numberNodes.length; j++) {
      if (numberNodes[j].classList.contains(String(numbers[_i]))) {
